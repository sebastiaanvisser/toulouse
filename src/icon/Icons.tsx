...
import { clipAsIcon, layers, mask, offsetAsIcon, Shape } from './Shape'

export type Icon = Once<Shape>
...
export const half = mk('half', () => {
  // "M0,12 C0,9.09765625 0,5.09765625 0,0 C4.64737216,1.66533454e-16 9.33815752,3.11387946 11,8 C11.6003287,9.76507604 12.5825733,11.2115741 16.203551,12 C11.940341,12 6.53915729,12 0,12 Z"
  const w = 10
  const h = 8
  const u = 8
  const v = 7
...
const play = mk('play', () =>
  S.poly(pt(0, -5), pt(0, 5), pt(6, 0))
    .dx(-3)
    .rounded() //
    .width(2)
)

const playPause = mk('playPause', () =>
  layers(
    play.get(),
    S.rect(1.5, 11)
      .rounded() //
...
const rewind = mk('fastForward', () => fastForward.get().scale2(-1, 1))

const playNext = mk('playNext', () =>
  layers(
    play.get(),
    S.rect(1.5, 10)
      .rounded() //
...
const playPrev = mk('playPrev', () => playNext.get().scale2(-1, 1))
const eject = mk('eject', () => playPause.get().rotate(-90))
const record = mk('record', () => S.circleShape(6))

export const repeat = mk('repeat', () =>
  layers(
    mask(S.circleShape(7).outline(2), pieSlice(-150, 90, 25)),
    S.circleShape(1).dx(7).rotate(150),
    S.poly(pt(0, -1), pt(1, 0), pt(0, 1))
      .scale(3)
      .width(1.5 / 3)
      .dy(-7)
...
export const haltOpen = mk('haltOpen', () =>
  layers(
...
export const Half = iconize(half)
...
export const Halt = iconize(halt)
...
export const HaltOpen = iconize(haltOpen)
...
export const Play = iconize(play)
export const PlayPause = iconize(playPause)
export const Pause = iconize(pause)
export const Stop = iconize(stop)
export const Rewind = iconize(rewind)
export const FastForward = iconize(fastForward)
export const PlayNext = iconize(playNext)
export const PlayPrev = iconize(playPrev)
export const Eject = iconize(eject)
export const Repeat = iconize(repeat)
export const Record = iconize(record)
