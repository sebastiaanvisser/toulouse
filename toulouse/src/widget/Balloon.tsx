...
import { clipAsIcon, Img, offsetAsIcon } from '../widget/Image'
...
type Props = BalloonProps & Omit<BoxProps, 'width' | 'height'>
...
export function Balloon_(props: Props & Context) {
  const {
...
function Handle(props: Props & { dim: Dimensions }) {
  const small = useResolvedeSmall(props)
  ...
  return (
    <div className={cx(handleC)} style={style}>
      <Img fg={Bg} img={small ? balloonHandleSmall : balloonHandle} />
    </div>
  )
}
